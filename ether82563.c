/* Until we know otherwise, this is released under the LPL */
/*
...
i82563ifstat(struct ether *edev, void *a, long n, unsigned long offset)
{
	...
	b = rbtab + ctlr->pool;
	p = seprintf(p, e, "pool: fast %ud slow %ud nstarve %ud nwakey %ud starve %ud\n",
...
static void
rbfree0(struct block *b)
{
	rbfree(b, 0);
}

static void
rbfree1(struct block *b)
{
	rbfree(b, 1);
}

static void
rbfree2(struct block *b)
{
	rbfree(b, 2);
}

static void
rbfree3(struct block *b)
{
	rbfree(b, 3);
}

static void
rbfree4(struct block *b)
{
	rbfree(b, 4);
}

static void
rbfree5(struct block *b)
{
	rbfree(b, 5);
}

static void
rbfree6(struct block *b)
{
	rbfree(b, 6);
}

static void
rbfree7(struct block *b)
{
	rbfree(b, 7);
}

static void
rbfree8(struct block *b)
{
	rbfree(b, 8);
}

static void
rbfree9(struct block *b)
{
	rbfree(b, 9);
}
...
static int
i82563replenish(struct ctlr *ctlr, int maysleep)
{
...
static unsigned int
phyread0(struct ctlr *c, int phyno, int reg)
{
	unsigned int phy, i;
...
static unsigned int
phywrite0(struct ctlr *c, int phyno, int reg, uint16_t val)
{
	unsigned int phy, i;
...
static void
phyerrata(struct ether *e, struct ctlr *c, unsigned int phyno)
{
	if(e->netif.mbps == 0){
		if(c->phyerrata == 0){
			c->phyerrata++;
...
i82563attach(struct ether *edev)
{
	...
	ctlr->nrd = Nrd;
	ctlr->ntd = Ntd;
	ctlr->alloc = kzmalloc(ctlr->nrd * sizeof(Rd) + ctlr->ntd * sizeof(Td) + 255, 0);
	if(ctlr->alloc == NULL){
		qunlock(&ctlr->alock);
		error(Enomem);
	}
...
static void
i82563interrupt(void*unused, void *arg)
{
	struct ctlr *ctlr;
	struct ether *edev;
	uint32_t icr, im;
...
static void
i82563shutdown(struct ether *edev)
{
...
static int
fcycle(struct ctlr*unused, Flash *f)
{
	uint16_t s, i;
...
static int
fread(struct ctlr *c, Flash *f, int ladr)
{
	uint16_t s;
...
static void
defaultea(struct ctlr *ctlr, uint8_t *ra)
{
	unsigned int i, r;
	uint64_t u;
	...
	if(cttab[ctlr->type].flag & Fflashea){
		/* intel mb bug */
		u = (uint64_t)csr32r(ctlr, Rah)<<32u | (unsigned int)csr32r(ctlr, Ral);
...
static long
i82563ctl(struct ether *edev, void *buf, long n)
{
	ERRSTACK(2);
	char *p;
	uint32_t v;
